  ...
  if (assessment.status === "valid") {
    return (
      <a
        ...
        target="_blank"
        rel="noreferrer"
        className="break-all text-foreground underline decoration-accent underline-offset-4 transition hover:decoration-accent"
      >
    ...
    <section className="rounded-2xl border border-border bg-card p-6 sm:p-7">
      ...
      <ol className="mt-6 space-y-4" aria-label="Evidence items timeline">
        {evidenceItems.map((item, index) => (
          <li key={item.id} className="relative rounded-[1.7rem] border border-border/80 bg-background p-5 pl-6 sm:pl-8">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3 pl-6 sm:pl-8">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="rounded-full border border-accent/30 bg-accent/5 px-3 py-1 font-mono text-[0.62rem] uppercase tracking-[0.22em] text-foreground">
                    {item.type}
                  </span>
            ...
            <dl className="mt-5 grid gap-4 pl-6 text-sm leading-6 sm:grid-cols-[minmax(0,0.72fr)_minmax(0,1.28fr)] sm:pl-8">
              <div className="rounded-[1.2rem] border border-border/70 bg-card p-4">
