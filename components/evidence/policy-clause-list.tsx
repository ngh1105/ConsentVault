  ...
  return (
    <section className="evidence-card p-6 sm:p-7">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          ...
          {matchedClauses.length > 0 ? (
            <ul className="mt-4 space-y-3" aria-label="Matched blocked clauses">
              {matchedClauses.map((clause) => (
                <li
                  key={clause}
                  className="rounded-[1.2rem] border border-accent/20 bg-card/75 px-4 py-3 text-sm leading-6 text-foreground"
                >
                  {clause}
                </li>
              ))}
        ...
        <article className="rounded-[1.6rem] border border-border/80 bg-background/70 p-5">
          <div className="flex items-center gap-2 text-accent">
            <ScrollText className="h-4 w-4" aria-hidden="true" />
            <p className="font-mono text-[0.68rem] uppercase tracking-[0.22em]">Full blocked-use list</p>
          </div>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <span>{clause}</span>
                    {isMatch ? (
                      <span className="rounded-full border border-accent/20 bg-accent/10 px-2 py-1 font-mono text-[0.62rem] uppercase tracking-[0.2em] text-accent">
                        Match
                      </span>
                    ) : null}
                  </div>
